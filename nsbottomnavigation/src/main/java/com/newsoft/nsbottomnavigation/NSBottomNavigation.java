...
import com.newsoft.nsbottomnavigation.notification.NSNotificationHelper;
 ...
 */
public class NSBottomNavigation extends FrameLayout {
	...
	private ArrayList<NSBottomNavigationItem> items = new ArrayList<>();
	private ArrayList<View> views = new ArrayList<>();
	private NSBottomNavigationBehavior<NSBottomNavigation> bottomNavigationBehavior;
	...
	private List<NSNotification> notifications = NSNotification.generateEmptyList(MAX_ITEMS);
	 ...
	 * Constructors
	 */
	public NSBottomNavigation(Context context) {
		super(context);
		init(context, null);
	}

	public NSBottomNavigation(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}

	public NSBottomNavigation(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void createClassicItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			NSBottomNavigationItem item = items.get(itemIndex);
			...
			if (itemsEnabledStates[i]) {
				...
				iconDrawable = forceTint ? NSHelper.getTintDrawable(items.get(i).getDrawable(context),
	...
	private void updateItems(final int itemIndex, boolean useCallback) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (i == itemIndex) {
				...
				icon.setSelected(true);
				NSHelper.updateTopMargin(icon, inactiveMarginTop, activeMarginTop);
				NSHelper.updateLeftMargin(notification, notificationInactiveMarginLeft, notificationActiveMarginLeft);
				NSHelper.updateTextColor(title, itemInactiveColor, itemActiveColor);
				NSHelper.updateTextSize(title, inactiveSize, activeSize);
				if (forceTint) {
					NSHelper.updateDrawableColor(context, items.get(itemIndex).getDrawable(context), icon,
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && colored) {
					...
					NSHelper.updateViewBackgroundColor(this, currentColor,
				...
				icon.setSelected(false);
				NSHelper.updateTopMargin(icon, activeMarginTop, inactiveMarginTop);
				NSHelper.updateLeftMargin(notification, notificationActiveMarginLeft, notificationInactiveMarginLeft);
				NSHelper.updateTextColor(title, itemActiveColor, itemInactiveColor);
				NSHelper.updateTextSize(title, activeSize, inactiveSize);
				if (forceTint) {
					NSHelper.updateDrawableColor(context, items.get(currentItem).getDrawable(context), icon,
	...
	private void updateSmallItems(final int itemIndex, boolean useCallback) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (i == itemIndex) {
				...
				if (titleState != TitleState.ALWAYS_HIDE) {
					NSHelper.updateTopMargin(icon, inactiveMargin, activeMarginTop);
					NSHelper.updateLeftMargin(notification, notificationInactiveMarginLeft, notificationActiveMarginLeft);
					NSHelper.updateTopMargin(notification, notificationInactiveMarginTop, notificationActiveMarginTop);
					NSHelper.updateTextColor(title, itemInactiveColor, itemActiveColor);
					NSHelper.updateWidth(container, notSelectedItemWidth, selectedItemWidth);
				}

				NSHelper.updateAlpha(title, 0, 1);
				if (forceTint) {
					NSHelper.updateDrawableColor(context, items.get(itemIndex).getDrawable(context), icon,
							itemInactiveColor, itemActiveColor, forceTint);
				}
				...
				if (titleState != TitleState.ALWAYS_HIDE) {
					NSHelper.updateTopMargin(icon, activeMarginTop, inactiveMargin);
					NSHelper.updateLeftMargin(notification, notificationActiveMarginLeft, notificationInactiveMarginLeft);
					NSHelper.updateTopMargin(notification, notificationActiveMarginTop, notificationInactiveMarginTop);
					NSHelper.updateTextColor(title, itemActiveColor, itemInactiveColor);
					NSHelper.updateWidth(container, selectedItemWidth, notSelectedItemWidth);
				}

				NSHelper.updateAlpha(title, 1, 0);
				if (forceTint) {
					NSHelper.updateDrawableColor(context, items.get(currentItem).getDrawable(context), icon,
							itemActiveColor, itemInactiveColor, forceTint);
				}
	...
	private void updateNotifications(boolean updateStyle, int itemPosition) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			final NSNotification notificationItem = notifications.get(i);
			final int currentTextColor = NSNotificationHelper.getTextColor(notificationItem, notificationTextColor);
			final int currentBackgroundColor = NSNotificationHelper.getBackgroundColor(notificationItem, notificationBackgroundColor);
			...
			if (updateStyle) {
				...
				if (notificationBackgroundDrawable != null) {
					...
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
						notification.setBackground(NSHelper.getTintDrawable(defautlDrawable,
								currentBackgroundColor, forceTint));
					} else {
						notification.setBackgroundDrawable(NSHelper.getTintDrawable(defautlDrawable,
								currentBackgroundColor, forceTint));
					}
				}
			}
	 ...
	 */
	public void addItemAtIndex(int index, NSBottomNavigationItem item) {
	 ...
	 * Add an item
	 */
	public void addItem(NSBottomNavigationItem item) {
		if (this.items.size() > MAX_ITEMS) {
			Log.w(TAG, "The items list should not have more than 5 items");
		}
		items.add(item);
	 ...
	 * Add all items
	 */
	public void addItems(List<NSBottomNavigationItem> items) {
	 ...
	 */
	public NSBottomNavigationItem getItem(int position) {
	...
	public void setBehaviorTranslationEnabled(boolean behaviorTranslationEnabled) {
		...
		if (getParent() instanceof CoordinatorLayout) {
			...
			if (bottomNavigationBehavior == null) {
				bottomNavigationBehavior = new NSBottomNavigationBehavior<>(behaviorTranslationEnabled, navigationBarHeight);
			} else {
	...
	public void manageFloatingActionButtonBehavior(FloatingActionButton fab) {
		if (fab.getParent() instanceof CoordinatorLayout) {
			NSBottomNavigationFABBehavior fabBehavior = new NSBottomNavigationFABBehavior(navigationBarHeight);
	...
	public void setNotification(int nbNotification, int itemPosition) {
        ...
        notifications.set(itemPosition, NSNotification.justText(title));
     ...
     * @param itemPosition int
     */
	public void setNotification(NSNotification notification, int itemPosition) {
		if (itemPosition < 0 || itemPosition > items.size() - 1) {
            throw new IndexOutOfBoundsException(String.format(Locale.US, EXCEPTION_INDEX_OUT_OF_BOUNDS, itemPosition, items.size()));
		}
        if (notification == null) {
            notification = new NSNotification(); // instead of null, use empty notification
        }
